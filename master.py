                    ...
                    for worker_id in select_workers:
                        ...
                        if data_to_send:  #check if data_to_send is not empty
                            ...
                            task = asyncio.create_task(session.post(
                                ...
                                json = {"Data": data_to_send, "Worker ID": worker_id}))  #create task to send data
                            tasks.append(task)
                    await asyncio.gather(*tasks)
                    for port in task_count:
                        print(f"Total tasks sent to worker{port}: {task_count[port]}")
                    lines = lines[3:]
        else: print("No workers are ready.")

        #if all data has been sent, print a message
        if not lines: print("No more data to send.")
...
@routes.post("/receive_word_count")
async def receive_word_count(request):
    ...
    word_count = data["Number of words"]
    ...
    elapsed_time = current_time - (time.monotonic_ns())  #calculate the elapsed time
    ...
    if worker_port in word_counts: word_counts[worker_port] += 1
    else: word_counts[worker_port] = 1
